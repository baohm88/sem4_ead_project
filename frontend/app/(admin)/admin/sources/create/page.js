...
import "@/styles/admin/SourceCreate.css";

// Giả sử bạn đã import toast ở đây:
...
export default function SourceForm({ editing, onSaved, categories }) {
    ...
    useEffect(() => {
        if (editing) {
            setForm({
                ...
                limit: editing.limit ?? 5, // [2] LẤY GIÁ TRỊ LIMIT CŨ KHI SỬA
            });
        } else {
            setForm(emptyForm);
        }
    ...
    const handleChange = (e) => {
        ...
        setForm((prev) => ({
            ...prev,
            // [3] XỬ LÝ KIỂU SỐ CHO status VÀ limit
            [name]: ["status", "limit"].includes(name) ? Number(value) : value,
        }));
    };
    ...
    const handleSubmit = async (e) => {
        ...
        const payload = {
            ...
            limit: form.limit, // [4] THÊM LIMIT VÀO PAYLOAD
    ...
    return (
        <div className="border rounded-lg p-4 bg-white shadow-sm">
            ...
            <form onSubmit={handleSubmit} className="space-y-3 text-sm">
                ...
                {/* [5] INPUT CHO TRƯỜNG LIMIT (Vị trí hợp lý, full width) */}
                <div>
                    <label className="block font-medium mb-1">Preview Limit (Links)</label>
                    <input
                        type="number"
                        name="limit"
                        value={form.limit}
                        onChange={handleChange}
                        className="w-full border rounded px-2 py-1"
                        placeholder="Số lượng link tối đa để preview"
                        required
                        min="1"
                    />
                </div>
